...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <p class="mb-4">Mengupdate data Workout.</p>
        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Edit Workout</h6>
            </div>
            <div class="card-body px-lg-5">
                <form action="/dashboard/workouts/{{ $workout->slug }}" enctype="multipart/form-data" method="post">
                    @csrf
                    @method('patch')
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9 col-sm-12 px-0">
                            <select class="form-control" id="category" name="category">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if ($workout->category_id == $category->id) selected @endif>{{ $category->title }}</option>
                                @endforeach
                            </select>
                            @error('category')
                                <div class="small text-danger">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9 col-sm-12 custom-file">
                            <input class="custom-file-input" type="file" id="file_cover" name="file_cover"
                                accept=".png,.jpg">
                            <label class="custom-file-label" for="file_cover">{{ $workout->photo }}</label>
                            @error('file_cover')
                                <div class="small text-danger">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9 col-sm-12 custom-file">
                            <input class="custom-file-input" type="file" id="file_poster" name="file_poster"
                                accept=".png,.jpg">
                            <label class="custom-file-label" for="file_poster">{{ $workout->poster }}</label>
                            @error('file_poster')
                                <div class="small text-danger">
                                    {{ $message }}
                                </div>
